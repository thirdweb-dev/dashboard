...
import type { DirectListingV3, MarketplaceV3 } from "@thirdweb-dev/sdk/evm";
...
const tableColumns: Column<DirectListingV3>[] = [
  {
    Header: "Listing Id",
  ...
  {
    Header: "Seller",
    accessor: (row) => row.listingCreatorAddress,
    Cell: ({ cell }: { cell: Cell<DirectListingV3, string> }) => (
  ...
  },
  {
    ...
    Cell: ({ cell }: { cell: Cell<DirectListingV3, any> }) => {
      return (
...
export const ListingsTable: React.FC<ListingsTableProps> = ({ contract }) => {
  ...
  const getActiveQueryResult = useDirectListings(contract, queryParams);
  ...
  const [tokenRow, setTokenRow] = useState<DirectListingV3 | null>(null);
