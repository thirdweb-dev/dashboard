...
export const PaymentsMintMethodInput: React.FC<PaymentsMintMethodInputProps> =
	({ abi, form }) => {
		...
		return (
			<Flex flexDir="column" gap={2} w="full">
				{filteredAbi.length === 0 ? (
					<Text>
						This contract doesn&apos;t have any function that can be used as
						Mint Method. Please use a different contract.
					</Text>
				) : (
					<AbiSelector
													...
													{({ isOpen }) => (
														<>
															...
															<MenuList zIndex={100}>
																...
																>
																	Enter <code>{input.name}</code>{" "}
																	argument&apos;s value or select a suggested
																	variable:
																</Text>
																<MenuItem
																	onClick={() => {
