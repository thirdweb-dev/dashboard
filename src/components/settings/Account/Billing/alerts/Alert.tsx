			...
			...(account?.recurringPaymentFailures?.map((failure) => {
				...
				return {
					...
					description: failure.subscriptionDescription
						? `${failure.subscriptionDescription} ${
								serviceCutoffDate ? `(due by ${serviceCutoffDate})` : ""
							}`
						: "",
					status: "error",
...
export const BillingAlertNotification: React.FC<BillingAlertNotificationProps> =
	({
		status,
		onDismiss,
		title,
