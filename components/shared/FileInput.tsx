...
export const FileInput: React.FC<IFileInputProps> = ({
  ...
  return (
    <Stack spacing={6} direction="row" w="100%" align="center">
      <AspectRatio ratio={1} w="150px">
        {isDisabled ? (
          <Box {...restBoxProps} cursor="not-allowed" opacity={0.5}>
          ...
          <Box {...restBoxProps} cursor="pointer" {...getRootProps()}>
            {noDisplay ? (
              <Stack align="center" color="gray.600">
                ...
                <Text color="gray.500">{fileType} uploaded</Text>
              </Stack>
            ) : fileUrl ? (
              <Image src={fileUrl} w="100%" />
            ) : (
              <Stack align="center" color="gray.600">
                <Icon boxSize={6} as={FiUpload} />
                <Text color="gray.500">Upload {helperText}</Text>
              </Stack>
            )}
            <input {...getInputProps()} />
          </Box>
